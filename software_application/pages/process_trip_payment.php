...
// Helper function to check for redeemed voucher
function isRedeemedVoucher($accountID, $promotionID, $db) {
    $stmt = $db->prepare("SELECT * FROM point_redemption WHERE accountID = ? AND itemID = ? AND itemType = 'Voucher' AND isUsed = 0 LIMIT 1");
...
if ($tripDetails) {
    ...
    if ($promotionID) {
        ...
        $voucher = isRedeemedVoucher($accountID, $promotionID, $db);
        if ($voucher) {
            $redemptionID = $voucher['redemptionID'];
            $isVoucher = true;
            // Get the discount from the promotion table using promotionID (itemID)
        ...
        if ($qty > 0) {
            ...
            if ($item && $item['stockQuantity'] >= $qty) {
                $merchandiseLineItems[] = [
                    'item' => $item,
                    'quantity' => $qty,
                    'subtotal' => $item['merchandisePrice'] * $qty
                ];
                $merchandiseTotal += $item['merchandisePrice'] * $qty;
            } else {
            ...
            if ($saleID) {
                ...
                foreach ($merchandiseLineItems as $line) {
                    if (!$lineOfSale->createNewLineOfSale(
                        ...
                        $line['quantity'],
                        $line['item']['merchandisePrice'],
                        $line['item']['merchandisePrice'] * $line['quantity']
                    )) {
                ...
                if ($isVoucher && $redemptionID) {
                    $stmt = $db->prepare("UPDATE point_redemption SET isUsed = 1 WHERE redemptionID = ? AND isUsed = 0 LIMIT 1");
        ...
        <tbody>
            ...
            <?php if (!empty($merchandiseLineItems)): ?>
                ...
                <tr>
                    ...
                    <td>RM<?php echo number_format($line['item']['merchandisePrice'], 2); ?></td>
